...
function isPopup(url) {
  return url.indexOf(chrome.runtime.id + '/src/popup.html') > -1
}
  ...
  if (port.name === 'bridge') {
    ...
    if (port.sender.tab) {
      ...
      url && url.indexOf('http') == 0
        && forge.tools.fire('tab-connected', {tab: id, url: url})
    ...
    if (isPopup(port.sender.url)) {
      var pid = POPUP_ID
      tabPorts[pid] = tabPorts[pid] || []
      tabPorts[pid].push(port)

      port.onDisconnect.addListener(function() {
        //console.log('closed port from tab', port.sender.tab.url)
        var ports = tabPorts[pid]
        ports.splice(ports.indexOf(port), 1)
      })
    }
    ...
    port.onMessage.addListener(function(message) {
      function processMessage(message) {
        ...
        function reply(content) {
          // send back reply
          // console.log('send reply', {content: content, callid: message.callid})
          port.postMessage({content: content, callid: message.callid}) // reply to tab
        }

        forge.tools.fire(message.type, message.content, reply, port.sender.tab ? port.sender.tab.id : -1)
      }

      // console.log('got message from tab', message)

      if (message.type == 'tab-connected') {
        forge.tabs.activeTab(function (tab) {
          message.content = {tab: POPUP_ID, url: tab.url}
 ...
 *
 * @param {integer} id of reciver tab
 * @param {string} type an arbitrary string which limits the listeners which will receive this message
 * @param {*} content the message body which will be passed into active listeners
 * @param {function} callback reply function
 * @param {function} error Not used.
 */
forge.message.sendTo = function(tabId, type, content, callback, error) {
  var ports = tabPorts[tabId]
  if (!ports.length) error({message: 'no ports on specified tabId'})

  var msg = {type: type, content: content}

  if (callback && typeof callback === 'function') {
    msg.callid = forge.tools.UUID()
    callbacks[msg.callid] = callback
  }
...
forge.message.broadcast = function(type, content, callback, error) {
  ...
  var processor = typeof content === 'function' && content

  chrome.windows.getAll({populate: true}, function (windows) {
    windows.forEach(function (win) {
      win.tabs.forEach(emitTabs)
    })
  })

  if (tabPorts.popup && tabPorts.popup.length) {
    forge.tabs.getCurrentTabUrl(function(url) {
      emitTabs({id: 'popup', url: url})
    })
  }
  ...
  function emitTabs(tab) {
    ...
    content = processor ? processor(tab) : content
    if (!content) return
...
var apiImpl = {
  message: {
    toFocussed: function(params, callback, error) {
      forge.tabs.activeTab(function (tab) {
        if (isPopup(tab.url)) return console.warn('toFocussed not allowed for popup when it open like regular tab', params)
  ...
  notification: {
    create: function(params, success, error) {
      chrome.notifications.create('', {
        ...
        success && chrome.notifications.onButtonClicked.addListener(function() {
          success()
  ...
  tabs: {
    ...
    activeTab: function (params, success) {
      chrome.tabs.query({active: true, lastFocusedWindow: true}, function(tabs) {
        if (!tabs || !tabs.length) {
          console.warn('Are you on bg page? Fallback to first active tab')
          return chrome.tabs.query({active: true}, function (tabs) { success(tabs[0]) })
        }
    ...
    onActiveTabChange: function(params, success) {
      function reply(tab) {
        ...
        var favIconUrl = /^chrome:/.test(tab.favIconUrl) ? null : tab.favIconUrl
        success({
          url: tab.url,
      ...
      chrome.tabs.onUpdated.addListener(function(tabId, reason, info) {
        forge.tabs.activeTab(function (tab) {
          if (tab.id !== tabId) return

          if (reason.url || reason.favIconUrl || reason.status === 'complete') {
            chrome.tabs.get(tabId, function(info) { reply(info) })
          }
        })
      })

      chrome.windows.onFocusChanged.addListener(function (windowId) {
        chrome.tabs.query({active: true, windowId: windowId, lastFocusedWindow: true}, function(tabs) {
          reply(tabs[0])
        })
      })

      forge.tabs.activeTab(function(tab) { reply(tab) })
    },
  ...
  button: {
    ...
    /*
     * Proper icon size: https://developer.chrome.com/extensions/browserAction#icon
     * NOTE: When setIcon() is called, it specifies the relative path to an image inside the extension
     */
    setIconByName: (function () {
      ...
      return function (name, success, error) {
        if (name == badgePrevPrefix) return success()
        ...
        chrome.browserAction.setIcon({
          path: {
            '19': './src/icon/icon19' + postfix + '.png',
            '38': './src/icon/icon38' + postfix + '.png'
          }
        })
        success()
      }
